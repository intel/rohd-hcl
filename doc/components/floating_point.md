...
The [FloatingPointConverter] will convert an arbitrary [FloatingPoint] and convert to another arbitrary [FloatingPoint]. It appropriately handles overflow and underflow, as well as subnormal to normal (or the reverse) when converting from a smaller exponent number to a larger one (or-vice-versa).  Rounding-nearest-even is the rounding mode used during conversion.

Options are:

- `ppTree`: The type of [PrefixTree] used in leading-one detect when going from smaller exponent to larger.
- `adderGen`: The type of [Adder] used for rounding the mantissa.

Here is an example of using the [FloatingPointConverter]:
