...
class FloatingPoint8Value extends FloatingPointValue {
  ...
  factory FloatingPointValue.fromFloatingPointValue(
    ...
    if (ingress_fpVal.isNaN()) {
      ...
      egressExponent = computeMaxExponent(egressExponentWidth) +
          computeBias(egressExponentWidth) +
          1;

      if (ingress_fpVal.mantissa.width <= egressMantissaWidth) {
        // Zero pad the mantissa in the case of precision gain
        egressMantissa = ingress_fpVal.mantissa <<
            (egressMantissaWidth - ingress_fpVal.mantissa.width);
      } else {
        // Trim the mantissa in the case of precision loss from the right end, preserving the left end
        egressMantissa = ingress_fpVal.mantissa.slice(
            ingress_fpVal.mantissa.width - 1,
            egressMantissaWidth - ingress_fpVal.mantissa.width);
      }

      return FloatingPointValue(
          sign: egressSign, exponent: egressExponent, mantissa: egressMantissa);
    }
    ...
    if (ingress_fpVal.isZero()) {
      return FloatingPointValue(
          sign: egressSign,
          exponent: LogicValue.zero(egressExponentWidth),
          mantissa: LogicValue.zero(egressMantissaWidth));
    }
    ...
    if (ingress_fpVal.isSubnormal()) {
      if (egressMantissa < ingress_fpVal.mantissa) {
        var mantissaWidthDiff =
            egressMantissa.width - ingress_fpVal.mantissa.width;
        var mask = ingress_fpVal.mantissa &
            LogicValue.filled(mantissaWidthDiff, LogicValue.one);

        // The lower bits of the mantissa and extracted and rounded using the sticky bit from the mask
        var lowMantissaBits =
            (ingress_fpVal.mantissa >>> mantissaWidthDiff) | (mask != 0);
        exgressExponent =
            LogicValue.filled(egressExponentWidth, LogicValue.zero);
    ...
    egressExponent = (ingress_fpVal.exponent.toInt() - ingress_fpVal.bias()) +
        FloatingPointValue.computeBias(egressExponentWidth);
  }
}
