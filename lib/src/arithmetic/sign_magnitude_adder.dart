...
/// A [SignMagnitudeAdderOld] performsa addition on values in sign/magnitude format.
class SignMagnitudeAdderOld extends Adder {
  ...
  /// [SignMagnitudeAdderOld] constructor with an adder functor [adderGen].
  ///
  ...
  /// comparator. Without the comparator, the [sign] may be wrong, but
  /// magnitude will be correct.
  // TODO(desmonddak): this adder may need a carry-in for rounding
  SignMagnitudeAdderOld(this.aSign, super.a, this.bSign, super.b,
    ...
    final adder = OnesComplementAdder(mux(sub, ~a, a), mux(sub, ~b, b),
        endAroundCarry: largestMagnitudeFirst ? Logic() : null,
        subtractIn: sub,
        adderGen: adderGen);
    sum <= adder.sum;
  }
