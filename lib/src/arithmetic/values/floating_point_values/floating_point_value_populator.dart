...
class FloatingPointValuePopulator<FpvType extends FloatingPointValue> {
  ...
  /// bound specifed by [gt] or [gte] and an upper bound specified by [lt] or
  /// [lte]. Only one of [gt] and [gte] can be specified, and only one of [lt]
  /// and [lte] can be specified.
  ///
  ...
  FpvType random(Random rv,
    ...
    final modGt = (gt == null)
        ? null
        : ((gt.isNormal() & (gt.sign == LogicValue.one) & !doGenNormal)
            ? cloneConstant(FloatingPointConstants.smallestPositiveNormal)
                .negate() as FpvType
            : ((!gt.isNormal() & (gt.sign == LogicValue.one) & !doGenSubNormal)
                ? cloneConstant(
                        FloatingPointConstants.smallestPositiveSubnormal)
                    .negate() as FpvType
                : (!gt.isNormal() &
                        (gt.sign == LogicValue.zero) &
                        !doGenSubNormal)
                    ? cloneConstant(
                        FloatingPointConstants.largestPositiveSubnormal)
                    : gt));

    final modGte = (gte == null)
        ? null
        : ((gte.isNormal() & (gte.sign == LogicValue.one) & !doGenNormal)
            ? cloneConstant(FloatingPointConstants.largestPositiveSubnormal)
                .negate() as FpvType
            : ((!gte.isNormal() &
                    (gte.sign == LogicValue.one) &
                    !doGenSubNormal)
                ? cloneConstant(FloatingPointConstants.positiveZero)
                : (!gte.isNormal() &
                        (gte.sign == LogicValue.zero) &
                        !doGenSubNormal)
                    ? cloneConstant(
    ...
    final modLt = (lt == null)
        ? null
        : ((lt.isNormal() & (lt.sign == LogicValue.zero) & !doGenNormal)
            ? cloneConstant(FloatingPointConstants.smallestPositiveNormal)
            : ((!lt.isNormal() & !doGenSubNormal)
                ? cloneConstant(FloatingPointConstants.largestPositiveSubnormal)
                    .negate() as FpvType
                : lt));
    final modLte = (lte == null)
        ? null
        : ((lte.isNormal() & (lte.sign == LogicValue.zero) & !doGenNormal)
            ? cloneConstant(FloatingPointConstants.largestPositiveSubnormal)
            : ((!lte.isNormal() & !doGenSubNormal)
                ? cloneConstant(FloatingPointConstants.smallestPositiveNormal)
                    .negate()
                : lte));

    final negNormals = doGenNormal &
        ((modGt ?? modGte)!.sign == LogicValue.one) &
        ((modGt ?? modGte)!.isNormal()) &
        (modGt?.neq(cloneConstant(FloatingPointConstants.smallestPositiveNormal)
                .negate()) ??
            true);

    final posNormals = doGenNormal &
        ((modLt ?? modLte)!.sign == LogicValue.zero) &
        ((modLt ?? modLte)?.isNormal() ?? true) &
        (modLt?.neq(
                cloneConstant(FloatingPointConstants.smallestPositiveNormal)) ??
            true);
    ...
    final posSubNormals = doGenSubNormal &
        ((modLt ?? modLte)!.sign == LogicValue.zero) &
        !(modLt?.isAZero ?? false) &
        !(modLte?.isAZero ?? false) &
        (modLt?.neq(cloneConstant(
                FloatingPointConstants.smallestPositiveSubnormal)) ??
            true);
    ...
    var gtSign = (modGt ?? modGte)?.sign;
    var gtMagnitude =
        (modGt != null) ? [modGt.exponent, modGt.mantissa].swizzle() : null;
    var gteMagnitude =
        (modGte != null) ? [modGte.exponent, modGte.mantissa].swizzle() : null;

    var ltSign = (modLt ?? modLte)?.sign;
    var ltMagnitude =
        (modLt != null) ? [modLt.exponent, modLt.mantissa].swizzle() : null;
    var lteMagnitude =
        (modLte != null) ? [modLte.exponent, modLte.mantissa].swizzle() : null;
    if ((!doGenSubNormal) & negNormals & posNormals) {
