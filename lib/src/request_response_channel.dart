  ...
  RequestResponseChannelBase({
    ...
    String? definitionName,
            ...
            definitionName: definitionName ??
                'RequestResponseChannelBase'
                    'ID${upstreamRequestIntf.data.id.width}'
                    'ADDR${upstreamRequestIntf.data.addr.width}'
...
class CachedRequestResponseChannel extends RequestResponseChannelBase {
  ...
  /// Cache read interface for address/data cache operations.
  late final ValidDataPortInterface cacheReadPort;

  /// Cache fill interface for address/data cache operations.
  late final ValidDataPortInterface cacheFillPort;

  /// CAM read interface for pending request tracking.
  late final ValidDataPortInterface camReadPort;

  /// CAM fill interface for pending request tracking.
  ...
  void _buildCacheLogic() {
    // Create internal logic signals with SystemVerilog-friendly names.
    ...
    canAcceptUpstreamReq <=
        ((cacheHit & internalResponseIntf.ready & ~responseFromDownstream) |
            ...
            .named('canAcceptUpstreamReqCombined');
    ...
    downstreamRequest.valid <=
        (upstreamRequest.valid &
                ...
                camSpaceAvailable)
            .named('downstreamRequestValidCombined');
    downstreamRequest.data <= upstreamRequest.data;
    ...
    final shouldStoreInCam = (upstreamRequest.valid &
            ...
            canForwardDownstream &
            camSpaceAvailable)
        .named('shouldStoreInCam');
    final shouldInvalidateCam =
        (downstreamResponse.valid & camHit).named('shouldInvalidateCam');
    ...
    final fifoWriteFromCache = (responseFromCache & ~responseFromDownstream)
        .named('fifoWriteFromCache');

    internalResponseIntf.valid <=
        (fifoWriteFromDownstream | fifoWriteFromCache)
            .named('internalResponseIntfValid');
