...
void main() {
  ...
  test('debug hit detection logic', () async {
    ...
    expect(initialHit, equals(0),
        reason: 'Initial read 0x10 should miss (cache empty)');
    readIntf.en.inject(0);
    await clk.nextPosedge;

    // Test 2: Fill first entry and verify hit
    // Filling 0x10 with 0xAA...
    ...
    expect(afterFillHit, equals(1),
        reason: 'After first fill, read 0x10 should hit');
    expect(afterFillData, equals(0xAA),
        reason: 'After first fill, read 0x10 should return correct data');
    readIntf.en.inject(0);
    await clk.nextPosedge;
    ...
    expect(differentAddrHit, equals(0),
        reason: 'Different address 0x20 should miss');
    readIntf.en.inject(0);
    await clk.nextPosedge;

    // Test 5: Now fill the second entry - this is where the bug happens
    // Filling 0x20 with 0xBB...
    fillIntf.en.inject(1);
    fillIntf.valid.inject(1);
    ...
    expect(addr100Hit, equals(1),
        reason: 'After second fill, read 0x10 should still hit');
    expect(addr100Data, equals(0xAA),
        reason: 'After second fill, read 0x10 should still '
            'return correct data');
    readIntf.en.inject(0);
    await clk.nextPosedge;
    ...
    expect(addr200Hit, equals(1),
        reason: 'After second fill, read 0x20 should hit');
    expect(addr200Data, equals(0xBB),
        reason: 'After second fill, read 0x20 should return correct data');
    readIntf.en.inject(0);
    await clk.nextPosedge;
